...
Chrome.prototype.close = function (callback) {
    ...
    function closeWebSocket(callback) {
        ...
        chrome._ws.close();
        chrome.once('disconnect', function () {
...
function connectToWebSocket(url) {
    ...
    return new Promise(function (fulfill, reject) {
        ...
        chrome._ws.onopen = function () {
            fulfill();
        };
        chrome._ws.onmessage = function (e) {
            // console.warn(JSON.stringify(data))
            const message = JSON.parse(e.data);
            // const message = data
            handleMessage.call(chrome, message);
        };
        chrome._ws.onclose = function (code) {
            chrome.emit('disconnect');
        }
        chrome._ws.onerror = function (err) {
            reject(err);
        }
    });
}
